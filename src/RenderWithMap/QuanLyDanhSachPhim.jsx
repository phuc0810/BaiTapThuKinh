...
export default class QuanLyDanhSachPhim extends Component {
    dataFilm = [
        {
            "maPhim": 1283,
            "tenPhim": "Trainwreck",
    ...
    renderFilms = () => {
        return this.dataFilm.map((film,index)=>{
            return <div className='col-2 mt-5' key={film.id}>
                <div className='card'>
                    <img src={film.hinhAnh} alt={film.biDanh} height={300} />
                    <div className='card-body bg-dark text-white'>
                        <p style={{height:35}} className='font-weight-bold'>{film.tenPhim}</p>
                        <p style={{height:100}}>{film.moTa.length > 100 ? film.moTa.substr(0,100) + '...' : film.moTa}</p>
                    </div>
                </div>
            </div>
        })
    ...
    render() {
        return (
            <div style={{ backgroundImage: 'url(./img/background/avanger.jpg)', backgroundPosition: 'center', width: '100wh', height: '100vh' }}>
                <div className='w-100 h-100' style={{ backgroundColor: 'rgba(0,0,0,.7)' }}>
                    <nav className="navbar navbar-expand-sm navbar-dark" style={{backgroundColor:'rgba(200,100,200,.2)'}}>
